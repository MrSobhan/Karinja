...
const SectionInsights = () => {
    ...
    return (
        <section className="pb-28 pt-14 container mx-auto lg:w-[80%] w-[90%]" dir="rtl">
            <div className="mt-14 grid gap-x-5 gap-y-8 md:grid-cols-2 lg:grid-cols-4">
                {stats.map((stat) => (
                    <div key={stat.id} className="flex flex-col gap-3 justify-center items-center">
                        <div className="text-5xl font-extrabold text-primary dana">{stat.value}</div>
                        <p className="text-muted-foreground dana">{stat.label}</p>
                    </div>
                ))}
            </div>
        </section>
